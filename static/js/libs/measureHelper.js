...
measureHelper.prototype.measureHeight=function () {
    ...
    this._handler.setInputAction(function (movement) {//此时界面上应该有三个点，三条线构成三角形
        ...
        var firstPointLatitude_Cartographic=Cesium.Cartographic.fromCartesian(_self._pointsCartesian3Array[0]).latitude;//第一个点的纬度
        var mouseCurrentPoint_Cartographic=Cesium.Cartographic.fromCartesian(_self._mouseCurrentCartesian);//当前鼠标点的制图坐标
        var secPointCartographic = new Cesium.Cartographic(mouseCurrentPoint_Cartographic.longitude,firstPointLatitude_Cartographic,mouseCurrentPoint_Cartographic.height);
        ...
        _self.addLabel_CallBack("firstLable", midPoint1, _self.getDistanceByPoint("空间距离：", _self._pointsCartesian3Array[0], _self._pointsCartesian3Array[2]));
        ...
        _self.addLabel_CallBack("thirdLable", midPoint3, _self.getDistanceByPoint("垂直距离：", _self._pointsCartesian3Array[1], _self._pointsCartesian3Array[2]));
        //增加坡度的计算
...
{
    //获得垂直距离
    this._geodesic.setEndPoints(Cesium.Cartographic.fromCartesian(this._pointsCartesian3Array[1]), Cesium.Cartographic.fromCartesian(this._pointsCartesian3Array[2]));
    var verticalDistance = Math.round(this._geodesic.surfaceDistance);
    ...
    return angelTxt = "坡度：" + (angle *180/ Math.PI).toFixed(2)+"度";
};
